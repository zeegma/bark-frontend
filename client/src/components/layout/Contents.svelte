    ...
    <section class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
      {#each items as item (item.id)}
        <div class="px-6 py-6 border border-stone-300 rounded-2xl bg-white">
          <div class="flex justify-between">
            ...
            <div class="flex flex-col flex-grow min-w-0">
              ...
              <p class="text-xs font-medium text-stone-400 truncate w-full">
                {categoryOptions.find((opt) => opt.value === item.category)
                  ?.label}
              </p>
            </div>
          ...
          <!-- Date and Time Found -->
          <div
            class="flex items-center gap-2 mb-1 text-xs font-medium text-stone-600"
          >
          ...
          <!-- Last Seen -->
          <div
            class="flex items-center gap-2 text-xs font-medium text-stone-600"
          >
